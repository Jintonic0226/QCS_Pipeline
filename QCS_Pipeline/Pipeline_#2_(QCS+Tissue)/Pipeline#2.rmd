...
num_cols <- ncol(overview_table_IS)

## Display the table in R Markdown
kable(overview_table_IS, format = "html", row.names = FALSE) %>%
  kable_styling(full_width = FALSE, position = "center") %>%
  add_header_above(c("Overview RSD Table IS" = num_cols)) 
```
:::
...
plot <- ggplot(pca_data, aes(x = PC1, y = PC2, color = Type, label = Sample)) +
  geom_point(size = 3) +
  geom_text(hjust = 1.5, vjust = 1.5, size = 5, check_overlap = TRUE, show.legend = FALSE) +
...
pca_plot_qcs <- ggplot(qcs_data, aes(x = PC1, y = PC2, color = Batch)) +
  geom_point(size = 3) +  # Plot QCS samples with color based on batch number
  geom_point(data = data.frame(PC1 = qcs_centroid[1], PC2 = qcs_centroid[2]), aes(x = PC1, y = PC2), shape = 17, fill = "black", color = "black", size = 3) +  # Plot centroid as triangle with filled red color
...
# Create a table plot
table_plot_qcs <- tableGrob(average_distance_qcs_df, theme = ttheme_minimal(base_size = 10), rows = NULL)
...
# Create a table plot
table_plot_qcs <- tableGrob(average_distance_TIC_qcs_df, theme = ttheme_minimal(base_size = 10), rows = NULL)
...
# Create table plot
table_plot <- tableGrob(tissue_avg_distances, theme = ttheme_default(base_size = 10))
...
# Create table plot
table_plot <- tableGrob(tissue_avg_distances_TIC, theme = ttheme_default(base_size = 8))
...
tissue_intergroup_plot <- ggplot(tissue_pca_data, aes(x = PC1, y = PC2, color = Type)) +
  geom_point(size = 3) +
  geom_point(data = centroids, aes(x = Centroid_PC1, y = Centroid_PC2, color = Type), shape = 18, size = 3) +
  geom_text_repel(data = centroids, aes(x = Centroid_PC1, y = Centroid_PC2, label = Type), 
                  direction = "both", size = 3) +  
  geom_text_repel(aes(label = Sample), direction = "both", size = 3) +  
...
centroid_table <- tableGrob(centroid_distances_df, theme = ttheme_default(base_size = 8))
...
# Create a table grob for centroid distances
centroid_table_TIC <- tableGrob(centroid_distances_df_TIC, theme = ttheme_default(base_size = 8))
...
<https://www.metaboanalyst.ca/MetaboAnnoalyst/upload/BatchUpload.xhtml>
:::
...
# Create overview table for Propranolol
overview_table_QCS <- data.frame(
...
cat("## Overview RSD Table QCS\n")
kable(overview_table_QCS, format = "html") %>%
  kable_styling(full_width = FALSE)

# Create overview table for D7-Propranolol
overview_table_IS <- data.frame(
  Row_Names = c("Intraday 1", "Intraday 2", "Intraday 3", "Interday"),
...
# Display table with title
cat("## Overview RSD Table Internal Standard\n")
kable(overview_table_IS, format = "html") %>%
  kable_styling(full_width = FALSE)
...
plot_violin <- function(data, title) {
  ...
  ggplot(data, aes(x = factor(Slide), y = Value, fill = factor(Batch))) +
    ...
    theme(
      ...
      axis.text.y = element_text(angle = 90, vjust = 0.5, hjust = 1, face = "bold", size = 10),
      axis.title = element_text(face = "bold", size = 15),  
...
pca_plot_qcs_combat <- ggplot(qcs_data_combat, aes(x = PC1, y = PC2, color = Batch)) +
  geom_point(size = 3) +  # Plot QCS samples with color based on batch number
  geom_point(data = data.frame(PC1 = qcs_centroid[1], PC2 = qcs_centroid[2]), aes(x = PC1, y = PC2), shape = 17, fill = "black", color = "black", size = 3) +  # Plot centroid as triangle with filled black color
...
# Create a table plot
table_plot_qcs_combat <- tableGrob(average_distance_qcs_df_combat, theme = ttheme_minimal(base_size = 10), rows = NULL)
...
# Create a table plot
table_plot_qcs_wave <- tableGrob(average_distance_qcs_df_wave, theme = ttheme_minimal(base_size = 10), rows = NULL)
...
# Add batch info
qcs_batch_info <- batch_info_normae[qcs_samples, "batch"] + 1 #since norm batch starts from 0
...
# Create a table plot
table_plot_qcs_norm <- tableGrob(average_distance_qcs_df_norm, theme = ttheme_minimal(base_size = 10), rows = NULL)
...
::: {.cell .r}
```{r fig.height=20, fig.width=7, echo=FALSE}
## Overview PCA PLOT ##
library(gridExtra)
...
# Create table plot
table_plot_combat <- tableGrob(tissue_avg_distances_combat, theme = ttheme_default(base_size = 8))
...
# Create table plot
table_plot_wave <- tableGrob(tissue_avg_distances_wave, theme = ttheme_default(base_size = 8))
...
# Create table plot
table_plot_norm <- tableGrob(tissue_avg_distances_norm, theme = ttheme_default(base_size = 8))
