...
# Package for VCA plot #
packages_vca <- c("VCA")

# Combine all package lists #
all_packages <- c(packages_RSD, packages_violin, packages_output, packages_excel, packages_vca)
...
cat("\033[1mOverview of RSD calculation for Internal Standard (No norm, TIC norm).\033[0m\n")
# Save HTML table to a file
display_html(display_rsd_table_pro_d7)
cat("\033[1mNotice: IS_norm is not an analyte specific (IS norm = QCS / Internal Standard).\033[0m\n")
```
:::
...
::: {#vca_plot0m .cell .markdown}
## \## 3. VCA Plot

<a id="vca_plot_1"></a>

Variability Chart for Analysis (VCA) plot is used in variance component
anaysis to visualze the variability of a dependent variable (QCS and IS
peak data) across the different levels of batch (row, slide, day).
:::
...
plot_vca_data <- function(data, qcs_mz_value, is_mz_value, plot_title) {
  ...
  # Replacing column name values with QCS/IS for subsetting
...
# VCA plots of no norm and TIC norm
no_normalization_vca <- plot_vca_data(propranolol_data, QCS_mz_value_1, IS_mz_value_1, "No Normalization Dataset (QCS)")
TIC_normalization_vca <- plot_vca_data(propranolol_data_TIC, QCS_mz_value_1, IS_mz_value_1, "TIC Normalization Dataset (QCS)")

# VCA Plot for Internal Standard Normalization #
# IS Dataset Upload
rawdata <- read.csv("Dataset/formatted_IS_norm_batch_data.csv",
                    row.names = 1,
                    check.names = FALSE
) 
qcs_batch_info_data <- rawdata 

# get rid of x infront of values
colnames(qcs_batch_info_data)
rownames(qcs_batch_info_data) 

# transpose
propranolol_data_IS <- as.data.frame(t(qcs_batch_info_data))
colnames(propranolol_data_IS) <- rownames(qcs_batch_info_data)
rownames(propranolol_data_IS) <- colnames(qcs_batch_info_data) 

propranolol_data_IS <- add_day_slide_row(propranolol_data_IS)
...
# QCS
IS_normalization_vca <- propranolol_data_IS %>%
  as.data.frame() %>%
...
plot_vca_data <- function(data, qcs_mz_value, is_mz_value, plot_title) {
  ...
  plot_data <- data %>%
    ...
    mutate(scale = scale(QCS))  # Scaling IS column
...
# VCA plots of no norm and TIC norm
no_normalization_vca_IS <- plot_vca_data(d7_propranolol_data, QCS_mz_value_1, IS_mz_value_1, "No Normalization Dataset (IS)")
TIC_normalization_vca_IS <- plot_vca_data(d7_propranolol_data_TIC, QCS_mz_value_1, IS_mz_value_1, "TIC Normalization Dataset (IS)")
```
:::

::: {#vca_plot3 .cell .r}
```{r}
# Display VCA Plot (QCS)
library(gridExtra)
options(repr.plot.width = 30, repr.plot.height = 8)  # Adjust width and height as desired
combined_vca_plot <- grid.arrange(no_normalization_vca, TIC_normalization_vca, IS_normalization_vca, ncol = 3)
```
:::

::: {#vca_plot4 .cell .r}
```{r}
# Display VCA Plot (Internal Standard)
library(gridExtra)
options(repr.plot.width = 30, repr.plot.height = 8)  # Adjust width and height as desired
combined_vca_plot_IS <- grid.arrange(no_normalization_vca_IS, TIC_normalization_vca_IS, ncol = 3)
```
:::
...
::: {.output .display_data}
'Excel/NO_Norm_QCS_batch_dataset_excel.xlsx'
:::

::: {.output .display_data}
'Excel/TIC_Norm_QCS_batch_dataset_excel.xlsx'
:::

::: {.output .display_data}
'Excel/IS_Norm_QCS_batch_dataset_excel.xlsx'
:::
...
overview_plots <- grid.arrange(combined_intensity_plot, combined_violin_plot, combined_violin_plot_d7, combined_vca_plot, combined_vca_plot_IS, ncol = 1)
display_html(display_rsd_table_pro)
display_html(display_rsd_table_pro_d7)
```
:::

::: {#results2 .output .display_data}
```{=html}
<h2 style='text-align: center;'>Overview RSD Table QCS</h2> <table class="table" style="width: auto !important; margin-left: auto; margin-right: auto;">
 <thead>
  <tr>
   <th style="text-align:left;"> Batch </th>
   <th style="text-align:left;"> No_norm </th>
   <th style="text-align:left;"> TIC_norm </th>
   <th style="text-align:left;"> IS_norm </th>
  </tr>
 </thead>
<tbody>
  <tr>
...
::: {.output .display_data}
```{=html}
<h2 style='text-align: center;'>Overview RSD Table Internal Standard</h2> <table class="table" style="width: auto !important; margin-left: auto; margin-right: auto;">
 <thead>
  <tr>
   <th style="text-align:left;"> Batch </th>
   <th style="text-align:left;"> No_norm </th>
...
<tbody>
  <tr>
   ...
   <td style="text-align:left;"> 66.959% </td>
  </tr>
  <tr>
   <td style="text-align:left;"> Intraday 2 </td>
   <td style="text-align:left;"> 39.711% </td>
   <td style="text-align:left;"> 38.576% </td>
  </tr>
  <tr>
   <td style="text-align:left;"> Intraday 3 </td>
   <td style="text-align:left;"> 35.566% </td>
   <td style="text-align:left;"> 36.631% </td>
  </tr>
  <tr>
   <td style="text-align:left;"> Interday </td>
   <td style="text-align:left;"> 53.147% </td>
   <td style="text-align:left;"> 53.139% </td>
  </tr>
</tbody>
</table>
```
...
::: {.output .display_data}
![](vertopal_20fe299090dc4c09b975252af89eaaf2/3589b1948b2860001b81a0e8bc0ce6316fab2d6d.png){height="1200"
width="1800"}
:::
::::::::::::::::::::::::::::::::::::

::: {#ea424496-c410-44ea-8b04-f88063a0c5a6 .cell .markdown}
## Saving the HTML tables and PNG plots
:::

::: {#fc8c7a8d-a5f9-4636-9865-73713989265e .cell .code execution_count="87" vscode="{\"languageId\":\"r\"}"}
``` r
library(kableExtra)
library(htmltools)
library(IRdisplay)
...
# Saving the VCA plot QCS to a file
file_path <- "Output/vca_QCS_plots.png"
ggsave(file_path, plot = combined_vca_plot, width = 40, height = 10, dpi = 300)

# Saving the VCA plot QCS to a file
file_path <- "Output/vca_IS_plots.png"
ggsave(file_path, plot = combined_vca_plot_IS, width = 40, height = 10, dpi = 300)
```
:::

::::: {#ff251986-c629-4dce-8196-848dddf0175a .cell .code execution_count="89" vscode="{\"languageId\":\"r\"}"}
``` r
...
```         
All png plots saved to separate pages within a single PDF file in the Output file.
```
:::
:::::
