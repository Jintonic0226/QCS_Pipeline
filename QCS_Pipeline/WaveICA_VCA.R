...
# Dataset Upload
rawdata <- read.csv("Pipeline_#2_(QCS+Tissue)/Dataset/waveica_interday_corrected_data.csv",
                    row.names = 1,
                    check.names = FALSE) #119x126
rawdata_transposed <-as.data.frame(t(rawdata))
rownames(rawdata_transposed) <- colnames(rawdata)
colnames(rawdata_transposed) <- rownames(rawdata)
...
row_names <- rownames(rawdata_transposed)
batch_info_norm <- batch_info_2
rawdata <- rawdata_transposed
...
# Adding Title
title(main = "WaveICA Dataset (QCS)")
