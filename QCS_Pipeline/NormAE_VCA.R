...
# Dataset Upload
rawdata <- read.csv("Pipeline_#2_(QCS+Tissue)/Dataset/Rec_nobe_no_norm.csv", row.names = 1 ) #119x126
sample_set_2 <- c("ChickenHeart", "ChickenLiver", "GoatLiver") ## change sample type names ### Preparing dataset
batch_info_2 <- read.csv("Pipeline_#2_(QCS+Tissue)/Dataset/Ys_no_norm.csv")
...
transposed_data$batch <- transposed_data$batch + 1
